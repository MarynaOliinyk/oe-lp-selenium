...
public class C1392CheckThatUserCanSelectDifferentLiveClassesTopicsTest extends DriverBase {
    ...
    public void checkThatThePracticeAndLessonsIconsAreLoadedTest() {
        ...
        page.loginPage.getPassword().shouldBe(visible).shouldHave(attribute(type, passwordText));
        page.loginPage.logIn(registeredEmail, correctPassword);
        page.inicioPage.getInicioLink().shouldBe(visible).shouldNotBe(empty).shouldHave(exactText(inicioLink));
        page.inicioPage.getUserName().shouldBe(visible).shouldNotBe(empty).shouldHave(text(userName));
