...
import static com.openenglish.core.TestData.TalkNow.PREGUNTE_A_UN_PROFESOR_TEXT;
import static com.openenglish.core.TestData.TalkNow.PREGUNTE_POPUP_INSTRUCTIONS_TEXT;
import static com.openenglish.core.TestData.TalkNow.PREGUNTE_POPUP_MESSAGE_PLACEHOLDER_TEXT;
...
public class C109VerifyEnviarUnaPreguntaContentTextTest extends DriverBase {
    ...
    public void verifyEnviarUnaPreguntaContentTextTest() {
        ...
        talkNowPage.getEnviarUnaPreguntaLink().shouldBe(visible, exactText(ENVIAR_UNA_PREGUNTA_TEXT)).click();
        talkNowPage.getPopupPreguntaTitle().shouldBe(visible, exactText(PREGUNTE_A_UN_PROFESOR_TEXT));
        talkNowPage.getPopupPreguntaInstructionsText().shouldBe(visible, exactText(PREGUNTE_POPUP_INSTRUCTIONS_TEXT));
        talkNowPage.getPopupPreguntaMessageBlock().shouldBe(visible, empty,
                attribute(PLACEHOLDER, PREGUNTE_POPUP_MESSAGE_PLACEHOLDER_TEXT));
        talkNowPage.getPopupPreguntaEnviarButton().shouldBe(visible, exactText(ASK_TEACHER_SUBMIT_TEXT),
                attribute(CLASS, ORANGE_BTN_DISABLED));
        talkNowPage.getPopupPreguntaMessageBlock().sendKeys(SOME_TEXT_FOR_TEST);
        talkNowPage.getPopupPreguntaEnviarButton().shouldBe(attribute(CLASS, ORANGE_BTN)).click();
        talkNowPage.getPopupGraciasTitle().shouldBe(visible, exactText(ALLERT_HEADER_TEXT));
        talkNowPage.getPopupGraciasMessageBlock().shouldBe(visible, exactText(PREGUNTE_POPUP_INSTRUCTIONS_TEXT));
        talkNowPage.getPopupGraciasCerrarButton().shouldBe(visible, exactText(ALLERT_CERRAR_TEXT),
