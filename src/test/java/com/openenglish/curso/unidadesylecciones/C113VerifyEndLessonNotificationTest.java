...
import static com.openenglish.core.TestData.Lessons.LO_LOGRO_FACEBOOK_BUTTON_URL;
import static com.openenglish.core.TestData.Lessons.LO_LOGRO_GOOGLE_BUTTON_URL;
import static com.openenglish.core.TestData.Lessons.LO_LOGRO_PROXIMA_BUTTON;
import static com.openenglish.core.TestData.Lessons.LO_LOGRO_SUS_LOGROS_TEXT;
import static com.openenglish.core.TestData.Lessons.LO_LOGRO_TITLE_TEXT;
import static com.openenglish.core.TestData.Lessons.LO_LOGRO_TWITTER_BUTTON_URL;
...
public class C113VerifyEndLessonNotificationTest extends DriverBase {
    ...
    public void lessonAreaSectionTest() throws InterruptedException, AWTException {
        ...
        lessonsPage.getBeginLessonButton().shouldBe(visible, exactText(BEGIN_LESSON_BUTTON)).click();
        lessonsPage.getLessonGreaterThenElement().shouldBe(exist, visible);
        executeJavaScript("window.parent.TCAPI.rate(5)");
        switchTo().defaultContent();
        lessonsPage.getPopUpLoLogroTitle().shouldBe(exist, visible, exactText(LO_LOGRO_TITLE_TEXT));
        ...
        lessonsPage.getPopUpLoLogroCompartaSusLogros().shouldBe(exist, visible, exactText(LO_LOGRO_SUS_LOGROS_TEXT));
        lessonsPage.getPopUpLoLogroTwitterLogo().shouldBe(exist, visible).click();
        switchTo().window(1);
        assertTrue(url().contains(LO_LOGRO_TWITTER_BUTTON_URL));
        switchTo().window(0);
        lessonsPage.getPopUpLoLogroFacebookLogo().shouldBe(exist, visible).click();
        switchTo().window(2);
        System.out.println(url());
        assertTrue(url().contains(LO_LOGRO_FACEBOOK_BUTTON_URL));
        switchTo().window(0);
        lessonsPage.getPopUpLoLogroGoogleLogo().shouldBe(exist, visible).click();
        switchTo().window(3);
        assertTrue(url().contains(LO_LOGRO_GOOGLE_BUTTON_URL));
        switchTo().window(0);
        lessonsPage.getPopUpLoLogroProximaLessonButton().shouldBe(exist, visible, exactText(LO_LOGRO_PROXIMA_BUTTON)).click();
